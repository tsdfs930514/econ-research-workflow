---
name: make-table
description: "Generate publication-quality LaTeX regression tables -- esttab, estout, three-line table (三线表), AER style, booktabs, multi-panel, first-stage, robustness summary, descriptive stats, balance table, multi-estimator comparison. Use when: 'make table', 'regression table', 'LaTeX table', 'esttab', 'format table', 'publication table', 'three-line table', '三线表'."
user_invocable: true
---
...
For the 5 compatibility rules (fragment mode with `\label{}`, avoiding duplicate headers, math-mode R², underscore escaping, Python `threeparttable` wrapping) and advanced patterns (nested tables via `\input{}`, `#delimit ;`, `stardrop()`, multi-estimator layout, auxiliary p-value files), read `references/make-table-advanced-patterns.md`.
