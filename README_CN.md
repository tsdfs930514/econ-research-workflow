...
- **38 个技能** — 覆盖完整研究生命周期的斜杠命令工作流（数据清洗、DID/IV/RDD/面板/SDID/自举/安慰剂/Logit-Probit/LASSO 估计、交叉验证、表格、论文写作、翻译、润色、去 AI 化改写、逻辑检查、审稿、管线编排、综合报告、CSMAR 数据获取、探索沙盒、会话连续性、苏格拉底式研究工具和自扩展）
...
| `/expert-review` | 专家评审 | 三位资深学者（资深教授、学术新星、严厉批评者）模拟评审，报告本地保存 |
| `/defend-paper` | 叙事防御 | 聚焦核心经济逻辑，四层框架（事实→机制→决策→后果）重排结果，砍掉堆砌 |
...
| 2026-03-13 | v0.20 | `/expert-review` 技能 — 三位资深学者模拟评审（资深教授、学术新星、严厉批评者），审稿报告本地保存 |
| 2026-03-13 | v0.21 | `/defend-paper` 技能 — 叙事防御，四层框架（事实→机制→决策→后果），结果纪律，审稿意见战略回应 |
